...
function fmtColor(color) {
  ...
  const prev = document.getElementById('tb-color-preview');
...
<div class="modal-body">
  ...
  <div class="form-group">
    <label class="form-label">Largeur du tableau</label>
    <select class="setting-select" id="tbl-width" style="width:100%">
      <option value="100%">Pleine largeur (100%)</option>
      <option value="75%">Large (75%)</option>
      <option value="50%">Moyen (50%)</option>
...
function doInsertTable() {
  ...
  const tWidth= document.getElementById('tbl-width')?.value || '100%';

  const thStyle = `border:2px solid #bbb;padding:8px 12px;min-width:60px;font-size:${fsize};background-color:${hBg};color:${hTxt};font-weight:700;text-align:left;`;
  const tdStyle = `border:1.5px solid #ddd;padding:7px 12px;min-width:60px;font-size:${fsize};background-color:${cBg};color:${cTxt};`;
  const tblLayout = tWidth === 'auto' ? 'auto' : 'fixed';

  let html = `<br><table style="border-collapse:collapse;width:${tWidth};margin:8px 0;table-layout:${tblLayout}"><tbody>`;
